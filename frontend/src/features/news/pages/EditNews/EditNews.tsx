                ...
                if (!content || !content.blocks || content.blocks.length === 0) {
                  return (
                    <p style={{ color: 'var(--text-secondary)', textAlign: 'center', fontStyle: 'italic' }}>
                      <i className="fas fa-info-circle" /> Nenhum conteúdo para visualizar.
                    </p>
                  );
                }
                    ...
                    case 'image':
                      return (
                        <div key={index} style={{ margin: '1.5rem 0', textAlign: 'center' }}>
                          <img src={block.data.file.url} alt={block.data.caption || ''} style={{ maxWidth: '100%', borderRadius: '8px' }} />
                          {block.data.caption && <p style={{ marginTop: '0.5rem', fontSize: '0.9rem', color: 'var(--text-secondary)', fontStyle: 'italic' }}>{block.data.caption}</p>}
                        </div>
                      );
                    case 'quote':
                      return (
                        <blockquote key={index} style={{ borderLeft: '4px solid var(--primary-color, #007bff)', paddingLeft: '1rem', marginLeft: 0, marginBottom: '1rem', fontStyle: 'italic', color: 'var(--text-secondary)' }}>
                          <p dangerouslySetInnerHTML={{ __html: block.data.text }} />
                          {block.data.caption && <footer style={{ marginTop: '0.5rem', fontSize: '0.9rem' }}>— {block.data.caption}</footer>}
                        </blockquote>
                      );
                    case 'code':
                      return <pre key={index} style={{ backgroundColor: '#f4f4f4', padding: '1rem', borderRadius: '4px', overflow: 'auto', marginBottom: '1rem' }}><code>{block.data.code}</code></pre>;
                    case 'table':
                      return (
                              ...
                              {block.data.content.map((row: string[], rowIndex: number) => (
                                <tr key={rowIndex}>
                                  {row.map((cell: string, cellIndex: number) => (
                                    <td key={cellIndex} style={{ border: '1px solid #ddd', padding: '0.5rem', textAlign: 'left' }} dangerouslySetInnerHTML={{ __html: cell }} />
                                  ))}
                                </tr>
                              ))}
                            </tbody>
                    ...
                    case 'embed':
                      return (
                        <div key={index} style={{ margin: '1.5rem 0' }}>
                          <iframe src={block.data.embed} width="100%" height="400" frameBorder="0" allowFullScreen style={{ borderRadius: '8px' }} title="Embed" />
                          {block.data.caption && <p style={{ marginTop: '0.5rem', fontSize: '0.9rem', color: 'var(--text-secondary)', textAlign: 'center' }}>{block.data.caption}</p>}
                        </div>
                      );
                    default:
